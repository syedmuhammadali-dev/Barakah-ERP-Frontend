  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">{t("zakat.description")}</p>
      </div>
          ...
          <Card className="bg-primary/5 border-primary/20 overflow-hidden relative">
            ...
            <CardContent className="pt-8">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                  <p className="text-sm font-medium text-primary uppercase tracking-wider mb-2">{t("zakat.currentObligation")}</p>
                  ...
                  {statusLoading ? <Skeleton className="h-4 w-64 mt-3" /> : (
                    <div className="flex items-center gap-2 mt-3">
                      <Badge variant={status?.isCompliant ? "default" : "destructive"} className={status?.isCompliant ? "bg-green-500/20 text-green-500 hover:bg-green-500/20" : ""}>
                        ...
                        {status?.isCompliant ? t("zakat.compliant") : t("zakat.reviewNeeded")}
                      </Badge>
                      <span className="text-sm text-muted-foreground">
                        {t("zakat.nextCalculation").replace("{days}", String(status?.nextCalculationDays ?? 0))}
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className="bg-card p-4 rounded-xl border border-border/50 shadow-sm min-w-50">
                  ...
                  <p className="font-medium text-sm">
                    {status?.lastCalculatedAt ? format(parseISO(status.lastCalculatedAt), 'MMMM d, yyyy') : t("zakat.never")}
                  </p>
                </div>
              </div>
            </CardContent>
          ...
          <Card>
            <CardHeader className="border-b border-border/50 pb-4">
              <CardTitle className="flex items-center gap-2">
                <History className="w-5 h-5" /> {t("zakat.historicalRecords")}
              </CardTitle>
            </CardHeader>
              ...
              {recordsLoading ? (
                ...
                <Table>
                  <TableHeader className="bg-muted/30">
                    <TableRow>
                      <TableHead>{t("zakat.date")}</TableHead>
                      <TableHead className="text-right">{t("zakat.netZakatable")}</TableHead>
                      <TableHead className="text-right">{t("zakat.zakatPaid")}</TableHead>
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader className="bg-secondary/20 border-b border-border/50 pb-4">
              <CardTitle className="flex items-center gap-2 text-lg">
                <Calculator className="w-5 h-5 text-primary" /> {t("zakat.calculateZakat")}
              </CardTitle>
              <CardDescription>{t("zakat.calculateDescription")}</CardDescription>
            </CardHeader>
          ...
          <Card className="bg-muted/30">
            <CardContent className="p-4 flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-medium">{t("zakat.complianceNote")}</p>
                <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
                  {t("zakat.complianceNoteText")}
                </p>
              </div>
            </CardContent>
          </Card>
          {statusError ? <p className="text-xs text-destructive">{t("zakat.zakatStatusUnavailable")}</p> : null}
        </div>
      </div>
    </div>
  );
